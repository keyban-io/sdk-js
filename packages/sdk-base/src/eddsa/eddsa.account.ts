import type { EddsaClientShare, EddsaWasmApi } from '~/eddsa/eddsa.types';
...
export class EddsaAccount {
  ...
  secretShare: EddsaClientShare['secret_share'] | null = null;

  static storagePrefix = 'KEYBAN-EDDSA';
   ...
   */
  constructor(
    clientKeyShare: EddsaClientShare,
    wasmApi: EddsaWasmApi,
    storage: StorageProviderApi<EddsaClientShare>,
  ) {
  ...
  async getClientShare(password?: string) {
    return this.accountStorageSolution.get(
      EddsaAccount.getStorageKey(this.keyId),
      password,
    );
  }
  ...
  static getStorageKey(keyId: string) {
    return `${EddsaAccount.storagePrefix}-${keyId}`;
  }
