Feature: Signing with the React Signer Library using EcDSA
    ...
    Background:
        ...
        And the library supports EcDSA signatures for Polygon

    @happy_path @data_signing @eds_dsa @generic
    Scenario: Successfully signing data with the master key using EcDSA
        Given the client has an existing master key using EcDSA
        When the client requests the library to sign data
        Then the library should return successfully signed data with EcDSA signatures
    ...
    Scenario: Handling errors during the data signing process with EcDSA
